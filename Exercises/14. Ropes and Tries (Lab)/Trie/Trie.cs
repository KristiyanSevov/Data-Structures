...
public class Trie<Value>
{
    ...
    private Node Insert(Node x, string key, Value val, int d)
    {
        ...
        if (d == key.Length) //root has no letter, so indices work
        {
            x.isTerminal = true;
            x.val = val;
            return x;
        ...
        Node node = null;
        char currentLetter = key[d];
        if (x.next.ContainsKey(currentLetter))
        {
            node = x.next[currentLetter];
        }
        x.next[currentLetter] = Insert(node, key, val, d + 1); 
        return x;
    }
